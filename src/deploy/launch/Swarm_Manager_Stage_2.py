#!/usr/bin/env python
from itertools import product
from actionlib.action_server import ros_timer
import rospy
import actionlib
from move_base_msgs.msg import MoveBaseAction, MoveBaseGoal
from rospy import client
import threading
import time
import datetime

from rospy.rostime import Time
from rospy.topics import Topic

process_units = [["red", "idle", -1.5, 2], ["green", "idle", 1.5, 0.7], ["blue", "idle", 5, 0.1],["drop_off", "idle", 0.25, 2.5],["home1", "idle", -2.5, 4],["home2", "idle", -2.5, 4.5], ["home3", "idle", -2.5, 3.5],["home4", "idle", -2.5, 5]]
product_units = [["Bot1", "idle"], ["Bot2", "idle"], ["Bot3", "idle"]]
tasks = []
tasks.append([["red", "pending", 3],["green", "pending", 2],["drop_off", "pending", 1],["home1", "pending", 1]])
tasks.append([["blue", "pending", 2],["green", "pending", 1],["drop_off", "pending", 1],["home2", "pending", 1]])
tasks.append([["red", "pending", 2],["blue", "pending", 1],["drop_off", "pending", 1],["home3", "pending", 1]])
tasks.append([["red", "pending", 3],["green", "pending", 2],["drop_off", "pending", 1],["home4", "pending", 1]])



rospy.init_node('movebase_client', anonymous=True)

def make_robot(x, y, robot): # Should be called move_robot.
    client = actionlib.SimpleActionClient(robot + '/move_base', MoveBaseAction)
    client.wait_for_server()

    goal = MoveBaseGoal()
    goal.target_pose.header.frame_id = "map"
    goal.target_pose.header.stamp = rospy.Time.now()
    goal.target_pose.pose.position.x = x
    goal.target_pose.pose.position.y = y
    goal.target_pose.pose.orientation.w = 1.0 

    client.send_goal(goal)
    wait = client.wait_for_result()
    if not wait:
        rospy.logerr("Action server not available!")
        rospy.signal_shutdown("Action server not available!")
        rospy.loginfo(client.get_result())

    else:
        rospy.loginfo(client.get_result())
    
    return client.get_result

def task_received(task, unit):
    location = [0, 0]
    job = len(task)
    while job > 0:
        for j in range(len(task)):
            for i in range(len(process_units)):
                test = task[j][0] in process_units[i] # We need better name than "test".
                if test == True and process_units[i][1] == "idle" and task[j][1] == "pending":
                    location[0] = process_units[i][2] 
                    location[1] = process_units[i][3]
                    done = "no"
                    while done == "no":
                        done = make_robot(location[0], location[1], unit[0])
                        job -= 1
                        task[j][1] = "complete"
    rospy.loginfo(unit + ": goal complete")
    unit[1] = "idle"
    return "idle"

while len(tasks) > 0:
    for i in range(len(product_units)):

        if product_units[i][1] == "idle":
            product_units[i][1] = "active"
            threading.Thread(target=task_received, args=(tasks[i], product_units[i])).start()
        if len(tasks) == 0:
            break